...
import 'package:medicoscope/utils/platform_utils.dart';
...
class _HeartMonitoringScreenState extends State<HeartMonitoringScreen>
  ...
  Future<void> _startRecording() async {
    ...
    if (kIsWeb) {
      // On web, record without specifying a path (records to blob)
      await _recorder.start(
        const RecordConfig(encoder: AudioEncoder.wav, numChannels: 1),
        path: '',
      );
    } else {
      final dir = await PlatformUtils.getTemporaryPath();
      final path =
          '$dir/heart_sound_${DateTime.now().millisecondsSinceEpoch}.wav';
  ...
  Future<void> _stopRecording() async {
    ...
    if (kIsWeb && path != null) {
      // On web, path is a blob URL — we need to fetch bytes from it
  ...
  Future<void> _pickFile() async {
    final result = await FilePicker.platform.pickFiles(
      ...
      withData: kIsWeb, // On web, get bytes directly
    );
    ...
    try {
      ...
      if (kIsWeb) {
        ...
        if (_selectedFileBytes != null) {
          ...
          // Blob URL from recording — fetch bytes via http package
          final response = await http.get(Uri.parse(_selectedFilePath!));
          bytes = response.bodyBytes;
        } else {
          throw Exception('No audio data available');
        }
        result = await CardioService.predictFromBytes(
          bytes,
          _selectedFileName ?? 'heart_sound.wav',
        );
      } else {
        result = await CardioService.predict(_selectedFilePath!);
      }
