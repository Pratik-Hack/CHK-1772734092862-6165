...
export default function PatientDashboard() {
  ...
  // Fetch alert count for badge like mobile app
      ...
      <div className="space-y-6">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="glass rounded-2xl p-6">
          ...
          <div className="flex gap-4 mt-4">
            ...
            {alertCount > 0 && (
              <Link href="/patient/alerts" className="px-4 py-2 rounded-xl bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-400 hover:bg-red-200 dark:hover:bg-red-900/50 transition-colors">
                <span className="text-sm">🔔 {alertCount} alert{alertCount !== 1 ? "s" : ""}</span>
              </Link>
            )}
          </div>
        </motion.div>
        ...
        <div className="glass rounded-2xl p-6">
          ...
          <div className="grid grid-cols-2 gap-3">
            {[
              ...
              { label: "Vital Alerts", href: "/patient/alerts", icon: "🔔", badge: alertCount },
              ...
              { label: "Claimed Rewards", href: "/patient/rewards/claimed", icon: "🎁" },
            ].map(link => (
              <Link key={link.href} href={link.href} className="flex items-center gap-2 p-3 rounded-xl hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors relative">
                <span>{link.icon}</span>
                <span className="text-sm font-medium">{link.label}</span>
                {link.badge !== undefined && link.badge > 0 && (
                  <span className="ml-auto bg-red-500 text-white text-xs font-bold px-1.5 py-0.5 rounded-full min-w-[18px] text-center">{link.badge}</span>
                )}
              </Link>
            ))}
          </div>
